...
worksheetDefinitions["base-amount-decimal"] = {
  label: "もとにする量（小数）",
  title: "割合からもとにする量を求める問題",
  instruction: "比べる量と割合から、もとにする量を求めましょう。",
  answerNote: "もとにする量＝比べる量÷割合です。割合は小数のまま使います。",
  type: "math",
  difficulties: {
    basic: {
      label: "基礎",
      ...
      counts: [5, 10, 15],
      description: "小数第1位までの割合を使って、もとにする量を求めます。"
    },
    standard: {
      label: "標準",
      ...
      counts: [5, 10, 15, 20],
      description: "小数第2位までの割合を使って、もとにする量を求めます。"
    },
    advanced: {
      label: "発展",
...
function formatBaseAmountRate(rateHundredths) {
  return (rateHundredths / 100).toFixed(2).replace(/0+$/, "").replace(/\.$/, "");
}

function buildBaseAmountDecimalPool(settings) {
  const pool = [];

  settings.rateHundredths.forEach((rateHundredths) => {
    const divisor = gcdBaseAmount(rateHundredths, 100);
    const baseUnit = 100 / divisor;
    const comparedUnit = rateHundredths / divisor;
    const rate = formatBaseAmountRate(rateHundredths);
    ...
    settings.multipliers.forEach((multiplier) => {
      ...
      if (Math.abs(compared / (rateHundredths / 100) - base) > 1e-9) return;
...
function makeBaseAmountDecimalWorksheet() {
  const definition = worksheetDefinitions["base-amount-decimal"];
  const settings = definition.difficulties[difficultySelect.value];
  const count = Number(countSelect.value);
  const pool = shuffle(buildBaseAmountDecimalPool(settings));
  ...
  for (const question of pool) {
    const key = `${question.compared}:${question.rate}`;
  ...
  selected.forEach((question, index) => {
    ...
    problem.textContent = `${number}. 比べる量 ${question.compared}、割合 ${question.rate}　もとにする量 □`;
    questionFragment.appendChild(problem);

    const answer = document.createElement("p");
    const main = document.createElement("b");
    main.textContent = `${number}. ${question.base}`;

    const explanation = document.createElement("span");
    explanation.className = "answer-explanation";
    explanation.textContent =
      `解説：もとにする量＝比べる量÷割合なので、` +
      `${question.compared}÷${question.rate}＝${question.base}。`;
  ...
  statusMessage.textContent = `もとにする量（小数）・${settings.label}を${count}問、重複なしで作成しました。`;
  document.querySelector(".problem-page").scrollIntoView({ behavior: "smooth", block: "start" });
}

createButton.addEventListener("click", (event) => {
  if (worksheetTypeSelect.value !== "base-amount-decimal") return;
  event.stopImmediatePropagation();
  makeBaseAmountDecimalWorksheet();
}, true);
