...
worksheetDefinitions["rate-line-diagram"] = {
  ...
  difficulties: {
    basic: {
      ...
      counts: [4, 6],
      description: "小数第1位までの割合を、線分図で読み取ります。"
    },
    standard: {
      label: "標準",
      ...
      counts: [4, 6, 8],
      description: "小数第2位までの割合で、3つの求め方を混合します。"
    },
    advanced: {
      label: "発展",
      ...
      multipliers: [1, 2, 3, 4, 5, 6],
      counts: [4, 6, 8],
      description: "1をこえる割合も含め、線分の長さの関係から判断します。"
    }
  }
};

function gcdRateLine(a, b) {
  let x = Math.abs(a);
  let y = Math.abs(b);
  while (y !== 0) {
    [x, y] = [y, x % y];
...
function formatRateLine(rateHundredths) {
...
function selectRateLineQuestions(relations, count) {
  const targets = rateLineTargets(count);
  const kinds = [];
...
function buildRateLineSvg(question) {
  ...
  const startX = 145;
  const maxLength = 390;
  const scale = maxLength / Math.max(1, question.numericRate);
  const baseLength = scale;
  const comparedLength = scale * question.numericRate;
  const baseEnd = startX + baseLength;
  const comparedEnd = startX + comparedLength;
  const baseY = 45;
  const comparedY = 100;
  ...
  [
    [startX, baseY], [baseEnd, baseY], [startX, comparedY], [comparedEnd, comparedY]
  ].forEach(([x, y]) => {
  ...
  const guideX = Math.max(baseEnd, comparedEnd) + 20;
  svg.append(
    makeSvgElement("line", { x1: guideX, y1: baseY, x2: guideX, y2: comparedY, class: "guide-line" }),
    makeSvgElement("text", { x: guideX + 12, y: 77, class: "rate-label" }, `割合 ${rateLineValueText(question, "rate")}`)
  );
...
function makeRateLineDiagramWorksheet() {
  const definition = worksheetDefinitions["rate-line-diagram"];
  ...
  selected.forEach((question, index) => {
    ...
    wrapper.className = "rate-line-question";

    const prompt = document.createElement("p");
    prompt.className = "rate-line-prompt";
    prompt.textContent = `${number}. □に入る数を求めましょう。`;
    wrapper.append(prompt, buildRateLineSvg(question));
    questionFragment.appendChild(wrapper);

    const data = rateLineAnswer(question);
